...
// 프로토타입은 프로퍼티를 읽을때만 사용하며 프로퍼티를 추가, 수정, 삭제하는 연산은 객체에 직접한다.
const student = {
    __proto__: user
}
...
메소드를 개체에서 호출했던, 프로토타입에서 호출했던 상관없이 언제나 this. 앞에 있는 객체이다.
*/
...
for (let prop in student) {
    ...
    /* 
    hasOwnProperty(키값)
    key에 대응하는 프로퍼티가 상속 프로퍼티가 아니고 obj에 직접 구현되어있는 프로퍼티일때만 true를 반환한다.
    */
